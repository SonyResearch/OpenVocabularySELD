...
class MetadataSynthesizer(object):
    def __init__(
        ...
        self._rnd_generator = np.random.default_rng(params['rng_seed'])
                ...
                for nmix in range(nb_mixtures):
                    ...
                    for nf in range(self._nb_frames):
                        ...
                        if nb_active > 0:
                            ...
                            active_files = metadata_nm['file'][active_events]
                            active_classes = metadata_nm['classid'][active_events]
                            active_tracks = metadata_nm['trackid'][active_events]
                            
                            # write to file
                            for na in range(nb_active):
                                freesoundid = os.path.splitext(os.path.basename(active_files[na][0]))[0]  # freesound id from audio file name
                                ...
                                metadata_writer.writerow([nf,classidx,trackidx,azim,elev,freesoundid])
                    file_id.close()
