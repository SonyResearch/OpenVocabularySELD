import os
from dcase2024_task3_seld_metrics import SELD_evaluation_metrics, cls_feature_class, parameters
import numpy as np
...
class ComputeSELDResults(object):
    def __init__(self, params, ref_files_folder=None, list_part_category=None):
        ...
        self._list_part_category = list_part_category
        ...
        for split in os.listdir(self._desc_dir):
            for ref_file in os.listdir(os.path.join(self._desc_dir, split)):
                ...
                gt_dict = self._feat_cls.load_output_format_file(os.path.join(self._desc_dir, split, ref_file),
                                                                 cm2m=True, list_part_category=self._list_part_category)  # TODO: Reconsider the cm2m conversion
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        if self.evaluate_distance:
            ...
            other_scores = eval.compute_seld_other_scores()
        ...
        if is_jackknife:
            ...
            return (ER, F, AngE, DistE, RelDistE, LR, seld_scr, classwise_results) if self.evaluate_distance \
                else (ER, F, AngE, LR, seld_scr, classwise_results, other_scores)
