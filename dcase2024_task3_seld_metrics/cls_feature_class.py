...
from dcase2024_task3_seld_metrics.cls_vid_features import VideoFeatures
...
#            pool.join()
    ...
    def load_output_format_file(self, _output_format_file, cm2m=False, list_part_category=None):  # TODO: Reconsider cm2m conversion
        """
        ...
        for _line in _fid:
            ...
            if list_part_category is not None:
                _words[1] = list_part_category.index(int(float(_words[1])))
    ...
    def organize_labels(self, _pred_dict, _max_frames):
        ...
        for frame_idx in range(0, _max_frames):
            ...
            for [class_idx, track_idx, az, el, *dist] in _pred_dict[frame_idx]:
                ...
                # Fix bug that the previous code dismiss parts of overlapping events of the same class and the same track_idx
                # E.g., the bug happens in pred since dummy track_idx 0 is used when len(_words) == 4 in load_output_format_file()
                if track_idx not in output_dict[frame_idx][class_idx]:
                    output_dict[frame_idx][class_idx][track_idx] = [az, el] + dist
                else:
                    # new_track_idx is set to a minus value to distinguish new_track_idx from raw track_idx (>= 0)
                    min_track_idx = np.min(np.array(list(output_dict[frame_idx][class_idx].keys())))
                    if min_track_idx >= 0:
                        new_track_idx = -1
                    else:
                        new_track_idx = min_track_idx - 1
                    output_dict[frame_idx][class_idx][new_track_idx] = [az, el] + dist
