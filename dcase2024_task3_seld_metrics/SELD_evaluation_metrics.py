...
class SELDMetricsSegmentLevel(object):
    ...
    def compute_seld_other_scores(self):
        '''
        Collect the final other SELD scores

        :return: returns both location-sensitive detection scores and class-sensitive localization scores
        '''
        ER_D = self._D / (self._Nref.sum() + eps)
        ER_I = self._I / (self._Nref.sum() + eps)
        ER_S = self._S / (self._Nref.sum() + eps)
        classwise_other_results = []
        if self._average == 'macro':
            # Location-sensitive detection performance
            Pre = self._TP / (eps + self._TP + self._FP_spatial + self._FP)
            Rec = self._TP / (eps + self._TP + self._FP_spatial + self._FN)

            # Class-sensitive localization performance
            LF = self._DE_TP / (eps + self._DE_TP + 0.5 * (self._DE_FP + self._DE_FN))
            LP = self._DE_TP / (eps + self._DE_TP + self._DE_FP)

            classwise_other_results = np.array([np.repeat(ER_D, self._nb_classes), np.repeat(ER_I, self._nb_classes), np.repeat(ER_S, self._nb_classes), Pre, Rec, LF, LP])
            Pre, Rec, LF, LP = Pre.mean(), Rec.mean(), LF.mean(), LP.mean()
        return ER_D, ER_I, ER_S, Pre, Rec, LF, LP, classwise_other_results
        ...
        for frame_cnt in range(len(gt.keys())):
            ...
            for class_cnt in range(self._nb_classes):
                ...
                if class_cnt in gt[frame_cnt] and class_cnt in pred[frame_cnt]:
                    ...
                    # Another bug happens when fixing bug of missing parts of overlapping events of the same class and the same track_idx
                    # Fix bug that the previous code output an error when err_list such as doa_err_list contains two or more
                    if not eval_dist:
                        Lc = np.sum(doa_err_list > self._ang_T)
                    else:
